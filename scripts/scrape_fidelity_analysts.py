...
def scrape_overview_data(driver, dropdown):
    ...
    urls = []
    for i in range(num_rows):
        # get url from appropriate attrirbute in table, add tag '&view=3' for 3 year view
...
def scrape_individual_data(driver):
    ...
    for i in range(num_rows):
        ...
        for j in range(num_cols):
            # getting text from the ith row and jth column
            row.append(driver.find_element_by_xpath(f'//table[{table_jsid}]/tbody/tr[{i + 1}]/td[{j + 1}]').text)
        sub_table.append(row)

    return sub_table

def scrape_analyst_data(driver, ticker):
    driver.get(f'https://snapshot.fidelity.com/fidresearch/snapshot/landing.jhtml#/analystsopinions?symbol={ticker.replace(".", "/")}')
    ...
    for i, url in enumerate(urls):
        ...
        try:
            ...
            flag = (datetime.now() - datetime.strptime(full_table[i][4], '%m/%d/%y')).days < 3*366
        except:
            pass
        if flag:
            driver.get(url)
...
if __name__ == '__main__':
    ...
    finished_tickers = [x.split('.')[0] for x in os.listdir('../pickles/ticker_data/success/') if 'pickle' in x]
