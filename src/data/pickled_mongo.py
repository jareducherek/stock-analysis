...
def mongo_eat_pkl(pkl_path, database, collection):
    ...
    overview_cols = pd.read_pickle(
        PICKLE_DIR / 'ticker_analyst_overview_column_names.pickle')
    historical_cols = pd.read_pickle(
        PICKLE_DIR / 'ticker_specific_analyst_column_names.pickle')
    historical_dfs = []
    ...
    for i in range(len(ticker_docs)):
        ...
        if len(historical_df) == 0:
            ...
            historical_df = pd.DataFrame(
                historical_df, columns=historical_cols)
        ticker_docs[i]['Historical Data'] = historical_df.to_dict(
            orient='records')
...
if __name__ == '__main__':
    ...
    ticker_files = [x.split('.pickle')[0]
                    for x in ticker_files if 'pickle' in x]
